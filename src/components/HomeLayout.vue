...
export default {
  ...
  data () {
    return {
      ...
      menuItens: [
        ...
        { name: 'Minhas Entregas', icon: 'mdi-moped-outline', to: '/proximasentregas' },
        { name: 'Programe um Trajeto Diário', icon: 'mdi-map-marker-path', to: '/trajeto' },
  ...
  methods: {
    isDisabled (item) {
      return item.name !== 'Pendências' && item.name !== 'Anúncios' && item.name !== 'Sair' && item.name !== 'Minhas Entregas' && item.name !== 'Programe um Trajeto Diário'
    }
  },
  mounted () {
